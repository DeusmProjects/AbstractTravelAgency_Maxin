...
namespace AbstractTravelAgencyServiceImplementDataBase.Implementations
{
        ...
        public void SaveVoucherCost(ReportBindingModel model)
        {
            ...
            try
            {
               ...
               missing);
                ...
                var products = context.Vouchers.ToList();
            //создаем таблицу
 var paragraphTable = document.Paragraphs.Add(Type.Missing);
               ...
               missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].VoucherName;
        ...
        public List<CitiesLoadViewModel> GetCitiesLoad()
        {
            return context.Cities
            .ToList()
           .GroupJoin(
            context.CityConditions
            .Include(r => r.Condition)
           .ToList(),
            stock => stock,
           stockCondition => stockCondition.City,
           (stock, stockCompList) =>
            new CitiesLoadViewModel
            {
        ...
        public List<CustomerBookingsModel> GetCustomerBookings(ReportBindingModel model)
        {
           ...
           .Where(rec => rec.DateCreateBooking >= model.DateFrom &&
           rec.DateCreateBooking <= model.DateTo)
        ...
        public void SaveCustomerBookings(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            ...
            table.AddCell(new PdfPCell(new Phrase("Путевка", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].DateCreateBooking, fontForCells));
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].VoucherName, fontForCells));
                table.AddCell(cell);
            ...
            cell = new PdfPCell(new Phrase(list[i].TotalSum.ToString(), fontForCells));
                cell.HorizontalAlignment = Element.ALIGN_RIGHT;
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].StatusBooking, fontForCells));
                table.AddCell(cell);
            }
            //вставляем итого
            ...
            table.AddCell(cell);
            cell = new PdfPCell(new Phrase(list.Sum(rec => rec.TotalSum).ToString(),
           fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_RIGHT,
                Border = 0
